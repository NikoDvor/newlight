...
async function runForClient(
  ...
  const { data: existing } = await supabase
    ...
    .maybeSingle();
  if (existing) return { client_id, skipped: true, reason: 'already_run', run_id: existing.id };

  const { data: deal } = await supabase
    .from('crm_deals')
    .select('id, pricing_model, commission_rate, recurring_fee, close_prep_completed_at')
    .eq('client_id', client_id)
  ...
  if (!deal || !deal.pricing_model) {
    const { data: run } = await supabase.from('commission_billing_runs').insert({
      ...
      pricing_model: 'retainer', amount_charged: 0, status: 'skipped',
      failure_reason: 'no completed close-prep deal',
    }).select('id').single();
  ...
  if (deal.pricing_model === 'retainer') {
    ...
    revenue_base = (wonDeals ?? []).reduce((s: number, d: any) => s + (Number(d.deal_value) || 0), 0);
  ...
  try {
    ...
    const { data: run } = await supabase.from('commission_billing_runs').insert({
      client_id, deal_id: deal.id, period_start, period_end,
      pricing_model: deal.pricing_model, revenue_base, rate_applied,
      amount_charged, stripe_payment_intent_id: pi.id,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body.client_id) {
      const result = await runForClient(supabase, body.client_id, period.period_start, period.period_end);
      return new Response(JSON.stringify({ period, result }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

    // No client_id → loop over all clients with a completed close-prep deal
    ...
    for (const cid of clientIds) {
      try {
        results.push(await runForClient(supabase, cid, period.period_start, period.period_end));
      } catch (err) {
        results.push({ client_id: cid, error: (err as Error).message });
      }
    }
