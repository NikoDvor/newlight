...
const getLevel = (score: number) => {
  if (score >= 90) return { label: "Excellent", color: "hsl(211 96% 56%)", icon: CheckCircle };
  if (score >= 70) return { label: "Healthy", color: "hsl(197 92% 48%)", icon: CheckCircle };
  if (score >= 50) return { label: "Needs Improvement", color: "hsl(211 80% 65%)", icon: AlertTriangle };
  return { label: "Critical", color: "hsl(222 68% 44%)", icon: XCircle };
};

export function HealthScoreWidget({ score, compact }: HealthScoreWidgetProps) {
        ...
        <div className="relative w-10 h-10">
          <svg viewBox="0 0 40 40" className="w-full h-full -rotate-90">
            ...
            <motion.circle cx="20" cy="20" r="16" fill="none" stroke={level.color} strokeWidth="3"
              strokeDasharray={`${score * 1.005} ${100.5 - score * 1.005}`}
              strokeLinecap="round"
        ...
        <div className="relative w-28 h-28">
          <svg viewBox="0 0 128 128" className="w-full h-full">
            ...
            <motion.circle cx="64" cy="64" r="56" fill="none" stroke={level.color} strokeWidth="8"
              strokeDasharray={`${score * 3.52} ${352 - score * 3.52}`}
