          ...
          <Dialog open={showManualAdd} onOpenChange={setShowManualAdd}>
            ...
            <DialogContent style={{ background: "hsl(218 35% 12%)", border: "1px solid hsla(211,96%,60%,.15)", color: "white" }}>
              ...
              <div className="space-y-3 mt-2">
                ...
                <div><label className="text-xs text-white/50 mb-1 block">Role preset</label><select value={manualRolePreset} onChange={e => { const v = e.target.value; setManualRolePreset(v); if (!["project_manager", "service_manager"].includes(v)) setManualClientId(""); }} className="w-full h-10 rounded-md bg-white/[0.06] border border-white/10 text-white text-sm px-3">{manualRoleOptions.map(role => <option key={role.value} value={role.value}>{role.label}</option>)}</select></div>
                {["project_manager", "service_manager"].includes(manualRolePreset) ? (
