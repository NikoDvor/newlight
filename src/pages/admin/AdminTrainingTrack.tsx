...
import { ScriptMemorizationVault, ScriptDefinition } from "@/components/training/ScriptMemorizationVault";
import { ProductKnowledgeOverview } from "@/components/training/ProductKnowledgeOverview";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { toast } from "@/hooks/use-toast";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip";
import { useModuleCompletion } from "@/hooks/useModuleCompletion";
import { useClientSop } from "@/hooks/useClientSop";
...
export default function AdminTrainingTrack({ basePath = "/admin/training-center" }: AdminTrainingTrackProps) {
  ...
  const isMeetingCadenceModule = selectedModule?.module_number === 7;
  const isProductKnowledgeModule = selectedModule?.module_number === 2;
            ...
            {loading ? (
              ...
              <>
                ...
                {(trackKey === "bdr" || trackKey === "salesmen" || !trackKey) && (
                  <>
                    <button
                      onClick={() => navigate(`${basePath}/bdr/certification`)}
                      className="w-full text-left px-4 py-3 transition-all duration-200 flex items-start gap-3 hover:bg-white/[0.03]"
                    >
                      <div className="h-7 w-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5 bg-secondary">
                        {hasCertification ? (
                          ...
                          <Award className="h-3.5 w-3.5 text-[hsl(var(--nl-neon))]" />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-1">
                          {hasCertification ? (
                            ...
                            <span className="text-[10px] text-[hsl(var(--nl-neon))] font-medium">Available</span>
                          )}
                        </div>
                      </div>
                    </button>
